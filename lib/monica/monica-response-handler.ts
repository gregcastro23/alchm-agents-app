...
export class MonicaResponseHandler {
  ...
  static compareCharacterVectors(
    userVector?: SignCharacterVector
  ): {
  ...
  static formatResponse(
    rawResponse: string,
    context: MonicaResponseContext
  ): MonicaResponse {
    const { emotionalState, primaryNeed, topicComplexity } = this.analyzeUserMessage(context.userMessage);
    ...
    const emotionalSupport = {
      ...
      gentle_challenge: emotionalState === 'neutral' ? this.generateGentleChallenge(context) : undefined
    };
    ...
    const ratings = {
      helpfulness: clamp(60 + (context.learningStage === 'beginner' ? 10 : 0) + (topicComplexity !== 'complex' ? 10 : 0)),
      personalization: clamp(40 + (context.userCharacterVector ? 20 : 0) + (context.currentAlchmQuantities ? 20 : 0)),
    ...
    const growthAttributes = {
      ...
      a_number_scaled: Math.max(0, Math.min(100, Math.round((context.currentAlchmQuantities?.aNumber || 0) / 40 * 100))),
    ...
    return {
      ...
      personality_context: {
        monica_character_vector_relevance: vectorComparison.commonalities.join('; ') || "Discovering our connection",
        user_character_vector_integration: vectorComparison.teachingOpportunities.join('; ') || "Exploring your unique gifts",
        current_cosmic_influence: context.currentCosmicWeather ? "Current transits support our work" : "Cosmic weather is calm"
      },
  ...
  private static extractConceptExplanation(response: string): string {
    ...
    const teachingSentence = sentences.find(s => 
      s.includes('means') || s.includes('represents') || s.includes('shows')
    );
  ...
  private static generatePracticalApplication(context: MonicaResponseContext): string {
    ...
    if (context.userCharacterVector) {
      const userEarth = context.userCharacterVector.taurus + context.userCharacterVector.virgo + context.userCharacterVector.capricorn;
      const userWater = context.userCharacterVector.cancer + context.userCharacterVector.scorpio + context.userCharacterVector.pisces;
      const userAir = context.userCharacterVector.gemini + context.userCharacterVector.libra + context.userCharacterVector.aquarius;
      const userFire = context.userCharacterVector.aries + context.userCharacterVector.leo + context.userCharacterVector.sagittarius;
  ...
  private static generateValidation(emotionalState: string, primaryNeed: string): string {
    ...
    return validations[emotionalState as keyof typeof validations] || "You're exactly where you need to be";
  }
