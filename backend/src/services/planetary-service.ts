...
export interface PlanetPosition { name: string; sign: string; degree: number; retrograde: boolean; house?: number }
export interface PlanetaryPositions { sun: PlanetPosition; moon: PlanetPosition; mercury: PlanetPosition; venus: PlanetPosition; mars: PlanetPosition; jupiter: PlanetPosition; saturn: PlanetPosition; uranus?: PlanetPosition; neptune?: PlanetPosition; pluto?: PlanetPosition }

export async function getCurrentPlanetaryPositions(date: Date = new Date()): Promise<PlanetaryPositions> {
  // Try real API first
  try {
...
function getFallbackPositions(date: Date): PlanetaryPositions {
  try {
    ...
    const calculatePosition = (orbitalDays: number, offset: number = 0): PlanetPosition => {
      ...
      return { name: '', sign: signs[signIndex], degree: Math.round(degree * 10) / 10, retrograde: Math.sin(progress * Math.PI * 2) < -0.8 }
    }
    ...
    Object.values(positions).forEach((planet, index) => { planet.house = ((index + houseOffset) % 12) + 1 })
    return positions
  } catch (error) {
