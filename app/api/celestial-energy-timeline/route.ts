...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!validation.valid) {
      ...
      } as CelestialEnergyResponse, { status: 400 })
    }
    ...
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Unknown error occurred during celestial energy calculation',
      performance: {
        calculationTime,
        cacheHitRate: 0,
        momentsPerSecond: 0
      }
    } as CelestialEnergyResponse, { status: 500 })
  }
}
  ...
  try {
    switch (action) {
      ...
      default:
        return NextResponse.json({
          success: false,
          error: 'Invalid action. Supported actions: live, intervals, locations, agent-degrees, cache-stats'
        }, { status: 400 })
    }
  } catch (error) {
...
function validateRequest(request: CelestialEnergyRequest): { valid: boolean; error?: string } {
  ...
  for (const metric of request.metrics) {
    if (!validMetrics.includes(metric)) {
      return { valid: false, error: `Invalid metric: ${metric}. Must be one of: ${validMetrics.join(', ')}` }
    }
  }
...
async function generateAgentActivations(moments: CelestialMoment[]): Promise<AgentActivation[]> {
  ...
  return activations
    ...
    .slice(0, 20) // Limit to top 20 activations
}
...
function generateAgentMessage(agent: any, moment: CelestialMoment, alignment: any): string {
  ...
  return messages[agent.id as keyof typeof messages] ||
...
function generateAgentWisdom(agent: any, moment: CelestialMoment): string {
  ...
  return wisdomTemplates[agent.id as keyof typeof wisdomTemplates] ||
    `The ${evolutionPhase} phase resonates at ${(resonance * 100).toFixed(1)}%, revealing cosmic wisdom.`
}

async function generateComparisonData(request: CelestialEnergyRequest, mainTimeSeries: CelestialTimeSeries) {
  ...
  const mainAvgResonance = mainTimeSeries.statistics.averageValues.consciousness?.resonanceLevel || 0
  const compAvgResonance = comparisonTimeSeries.statistics.averageValues.consciousness?.resonanceLevel || 0
...
async function handleIntervalInfo() {
  return NextResponse.json({
    ...
    data: {
      ...
      metrics: [
        { value: 'A#', label: 'Alchemical Number', description: 'Primary consciousness-energy metric' },
        { value: 'SMES', label: 'Spirit-Matter-Essence-Substance', description: 'Foundational alchemical elements' },
        { value: 'kinetic', label: 'Kinetic Properties', description: 'Velocity, momentum, power, and inertia' },
        { value: 'thermo', label: 'Thermodynamic Values', description: 'Heat, entropy, reactivity, and energy' }
      ]
    }
  })
}
