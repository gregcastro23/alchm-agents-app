...
async function testMonicaTarot() {
  try {
    const response = await fetch('http://localhost:3000/api/monica-agent', {
      ...
      body: JSON.stringify({
        message: "Monica, please explain The Fool card in detail, including its alchemical properties, chakra associations, and give me a reading with this card for my current situation.",
        includeAlchm: true
      })
    });
