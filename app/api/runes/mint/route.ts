...
export async function POST(req: Request) {
  try {
    ...
    if (!targetRune) {
      return NextResponse.json(
        { success: false, error: 'Rune not found' },
        { status: 404 }
      );
    }
      ...
      return NextResponse.json(
        { 
          ...
          details: {
            ...
            requiredChartRange: [targetRune.chartRequirements.minCharts, targetRune.chartRequirements.maxCharts]
          }
        },
        { status: 400 }
      );
    ...
    const actualCosts = calculateMultiChartRuneCosts(targetRune, chartCombination, currentConditions);
    ...
    const mintedRune: MintedRune = {
      ...
      activeEffects: [
        ...targetRune.effects.map(effect => ({
          ...
          expiresAt: effect.duration !== 'permanent' && effect.duration !== 'instant' 
      ...
      collectiveParticipants: chartCombination.charts.length > 1 
...
function generateParticipantContributions(chartCombination: ChartCombination) {
  ...
  return chartCombination.charts.map(chart => ({
    ...
    contributionPercentage: Math.round(
      ((chart.alchmSignature?.aNumber || 0) / totalANumber) * 100
    )
  }));
}
...
function generateMonicaInsight(mintedRune: MintedRune, chartCombination: ChartCombination): string {
  ...
  if (synergy > 80) {
    insights.push("Exceptional harmony between charts has created a truly powerful consciousness tool.");
  } else if (synergy > 60) {
    insights.push("Good compatibility has enhanced this rune's effectiveness beyond its base power.");
  }
  
  if (mintedRune.collectiveParticipants) {
    insights.push(`All ${mintedRune.collectiveParticipants.length} participants will benefit from the collective effects.`);
  }
 ...
 */
function generateRecommendations(mintedRune: MintedRune, chartCombination: ChartCombination): string[] {
  const recommendations: string[] = [];
  const rune = mintedRune.originalRune;
  
  recommendations.push(`Allow ${mintedRune.enhancedProperties.craftingTime} minutes for the rune to fully activate`);
  ...
  if (mintedRune.enhancedProperties.cooldownRemaining) {
    recommendations.push(`Wait ${mintedRune.enhancedProperties.cooldownRemaining} hours before minting another ${rune.rarity} rune`);
  }
