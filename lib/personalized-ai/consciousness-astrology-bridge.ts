...
export function correlateConsciousnessToAstrology(profile: ConsciousnessProfile): CorrelatedAstrology {
  ...
  const fireScore = normalize(
    d.assertiveness + profile.creativity.creativity_level + comm.directness + think.processing_speed === 'fast_decisive' ? 15 : 0
  )
  const airScore = normalize(
    d.openness + comm.verbosity + (think.detail_big_picture) + (think.analytical_intuitive) + (values.innovation_tradition ? values.innovation_tradition : 0)
  )
  const earthScore = normalize(
    d.conscientiousness + (comm.formality) + (think.structured_flexible < 50 ? (100 - think.structured_flexible) : 0) + (behavior.routine_spontaneous < 50 ? (100 - behavior.routine_spontaneous) : 0)
  )
  const waterScore = normalize(
    comm.emotional_expression + (100 - d.emotional_stability) + (values.material_spiritual > 50 ? (values.material_spiritual - 50) : 0) + (profile.creativity.writing_style?.tone ? 10 : 0)
  )
  ...
  const dominant_element = (Object.entries(elementMap).sort((a,b)=> b[1]-a[1])[0][0]) as CorrelatedAstrology['dominant_element']
  ...
  if ((judging > 60 && (think.processing_speed === 'fast_decisive' || d.assertiveness > 60))) dominant_modality = 'Cardinal'
  ...
  if (comm.directness > 60 || think.analytical_intuitive < 50) planetary_tendencies.push('Mercury clarity')
