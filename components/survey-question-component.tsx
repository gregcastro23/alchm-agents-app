...
export function SurveyQuestionComponent({ question, value, confidence, onChange }: SurveyQuestionComponentProps) {
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4].slice(0, question.options.length).map(rank => (
                        <Button
                          ...
                          disabled={currentRanking.length >= rank && currentRanking[rank - 1] !== undefined}
...
function ConfidenceRating({ value, onChange }: { value: number; onChange: (value: number) => void }) {
  return (
