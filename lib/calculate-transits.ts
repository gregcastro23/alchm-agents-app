...
export function calculatePlanetPosition(
  ...
  const signs = Object.values(signIndices)
  const newSign = Object.keys(signIndices)[newSignIndex]
  
  return {
    sign: newSign,
    degree: Math.round(newDegree)
  }
}

// Calculate current positions based on the reference data from May 2024
export function getCurrentPlanetaryPositions(): Record<string, { sign: string, degree: string }> {
