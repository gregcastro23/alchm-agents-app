...
export function useGalileoLog(defaultComponentName?: string) {
  ...
  const info = useCallback((message: string, metadata?: Record<string, any>, componentName?: string) => 
    log(message, { metadata, level: 'info', componentName }), [log]);
    
  const warn = useCallback((message: string, metadata?: Record<string, any>, componentName?: string) => 
    log(message, { metadata, level: 'warning', componentName }), [log]);
    
  const error = useCallback((message: string, metadata?: Record<string, any>, componentName?: string) => 
    log(message, { metadata, level: 'error', componentName }), [log]);
    
  const debug = useCallback((message: string, metadata?: Record<string, any>, componentName?: string) => 
    log(message, { metadata, level: 'debug', componentName }), [log]);
