...
import { GregsEnergyCalculator, PlanetaryInfluenceCalculator, AdvancedConstantsCalculator } from './core-energy-rules';
import { 
...
export class PlanetaryRulesSystem {
  ...
  static validateCalculations(results: any): {
    ...
    if (results.energyAnalysis?.thermodynamics) {
      ...
      if (diff > 0.001) {
        errors.push(`Greg's Energy calculation mismatch: expected ${expectedGregsEnergy}, got ${energy}`);
        isValid = false;
      }
    }
    ...
    if (results.foodRecommendations) {
      ...
      const opposingTerms = ['opposing', 'opposite', 'balance fire with water', 'balance earth with air'];
      opposingTerms.forEach(term => {
        if (reasoning.toLowerCase().includes(term)) {
          warnings.push(`Elemental logic warning: found "${term}" - elements should not be treated as opposing`);
        }
      });
    }
  ...
  static performSystemHealthCheck(): {
    ...
    try {
      const testChart = {
        ...
        alchemyEffects: { spirit: 0, essence: 0, matter: 0, substance: 0, dayEssence: 0, nightEssence: 0, totalPower: 0 },
