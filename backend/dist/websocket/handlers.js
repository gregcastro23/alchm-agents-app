...
class WebSocketManager {
    wss;
    clients = new Map();
    channels = new Map(); // channel -> set of client IDs
    ...
    handleSubscription(clientId, channel, data) {
        const client = this.clients.get(clientId);
        if (!client)
            return;
        // Validate channel
        ...
        try {
            ...
            switch (data?.action) {
                case 'getCurrentPlanetaryHour':
                    if (client.location) {
                        responseData = await planetaryHoursService.getCurrentPlanetaryHour(data.datetime ? new Date(data.datetime) : new Date(), client.location);
                    }
                    else {
