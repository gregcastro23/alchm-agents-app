...
router.post('/historical', [
  ...
  historicalData.forEach(point => {
    Object.keys(tokenStats).forEach(token => {
      ...
      tokenStats[token as keyof typeof tokenStats].min = Math.min(tokenStats[token as keyof typeof tokenStats].min, value)
      tokenStats[token as keyof typeof tokenStats].max = Math.max(tokenStats[token as keyof typeof tokenStats].max, value)
