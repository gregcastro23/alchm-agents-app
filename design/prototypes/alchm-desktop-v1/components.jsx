...
function ElementGlyph({ element, size = 12 }) {
  ...
  const sw = 1.2;
  const p = {
    Fire:  <polygon points="6,1.5 11,10 1,10" fill="none" stroke={stroke} strokeWidth={sw} strokeLinejoin="round" />,
    Water: <polygon points="6,10.5 11,2 1,2"  fill="none" stroke={stroke} strokeWidth={sw} strokeLinejoin="round" />,
    Earth: (<g fill="none" stroke={stroke} strokeWidth={sw} strokeLinejoin="round">
              <polygon points="6,10.5 11,2 1,2" />
              <line x1="2.6" y1="6.5" x2="9.4" y2="6.5" />
            </g>),
    Air:   (<g fill="none" stroke={stroke} strokeWidth={sw} strokeLinejoin="round">
...
function Coin({ slot, value }) {
  ...
  return (
    <div className="coin" style={{ color: meta.hex }}>
      <div className="coin-glyph"><CoinIcon kind={meta.icon} size={13} /></div>
  ...
  return (
    <div className="harmonics">
      <div className="harmonic">
        ...
        <div className="value"><i>A♯</i>{monica.toFixed(4)}</div>
      </div>
      <div className="harmonic">
        <div className="label">Kalchm K</div>
        <div className="value"><i>K</i>{kalchm.toFixed(4)}</div>
      </div>
    </div>
  );
}
...
function Sacred7Chart({ stats }) {
  ...
  const polyPts = SACRED_7.map((s, i) =>
    valPt(i, stats[s.key] || 0)).map((p) => `${p.x},${p.y}`).join(' ');
  ...
  return (
    <svg className="sacred7-svg" viewBox="0 0 230 230" aria-label="Sacred 7 archetypes">
      ...
      {rings.map((r, i) => (
        <polygon key={i}
          points={SACRED_7.map((_, j) => valPt(j, r)).map((p) => `${p.x},${p.y}`).join(' ')}
          fill="none" stroke="rgba(237, 228, 210, 0.07)" strokeWidth="0.5" />
      ))}
      {/* axes */}
      {SACRED_7.map((_, i) => {
        const a = axisPt(i);
        return <line key={i} x1={cx} y1={cy} x2={a.x} y2={a.y}
                     stroke="rgba(237, 228, 210, 0.06)" strokeWidth="0.5" />;
      })}
      {/* value polygon */}
      ...
      {PLANETARY_12.map((p) => {
        ...
        return (
          <div key={p.key} className="planet-row">
            <span className="planet-name">{p.glyph} {p.label}</span>
...
function BlueprintMix({ blueprint, dominantElement }) {
  ...
  return (
    <div className="blueprint">
      ...
      }}>Alchemical Blueprint</div>
      <div className="blueprint-bar">
        {order.map((el) => (
          <div key={el} className="seg"
      ...
      <div className="council-overlay" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="council-metrics">
          ...
          <div className="cm-cell">
            ...
            <div className="val" style={{ color: ELEMENTS[dominant].hex }}>{dominant}</div>
          </div>
          <div className="cm-cell">
            ...
            if (!agent) {
              return (
                <div key={r.key} className="slot empty">
                  <div className="role" style={{ color: 'var(--muted)' }}>{r.label}</div>
                  <span>+ summon</span>
                </div>
              );
            }
              ...
              <div key={r.key} className="slot">
                ...
                <div className="agent" style={{ color: ELEMENTS[agent.element].hex }}>
                  ...
                  <div className="info">
                    ...
                    <span className="el">{agent.element} · {agent.model}</span>
                  </div>
                </div>
              </div>
            );
...
Object.assign(window, {
  ElementGlyph, ElementChip, AlchSlider, Coin, CoinIcon, Wallet,
  LevelTrack, HarmonicsReadout, HistoricalDietCard,
  Sacred7Chart, Sacred7Legend, Planetary12, BlueprintMix,
  CouncilOverlay, ROLES,
});
