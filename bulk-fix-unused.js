import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Common unused imports to remove (based on error analysis)
const unusedImports = [
  // Agent-related imports
  "getTransitsByPlanet", "HistoricalTransitCard", "ConsciousnessMemorySystem",
  "KineticProfile", "calculateMomentumSynergy", "calculateElementCompatibility",
  "calculateAstrologyCompatibility", "birthData", "agentProfile", "updateData",
  "generateAlchmForBirthInfo", "session",

  // Chart-related imports
  "ApiErrorBanner",

  // API route imports
  "req"
];

// Files to focus on (highest error counts)
const targetFiles = [
  'app/agents/[planet]/[sign]/[degree]/page.tsx',
  'app/alchm-quantities/page.tsx',
  'app/api/admin/system-stats/route.ts',
  'app/api/advanced-agent/route.ts',
  'app/api/agent-cache-metrics/route.ts',
  'app/api/agent-dashboard/route.ts',
  'app/api/agent-evolution/compatibility/route.ts',
  'app/api/agent-evolution/route.ts',
  'app/api/agent-interaction/route.ts',
  'app/api/agent-recommendations/route.ts'
];

function removeUnusedImports(filePath) {
  const fullPath = path.join(__dirname, filePath);
  if (!fs.existsSync(fullPath)) return false;

  let content = fs.readFileSync(fullPath, 'utf8');
  let modified = false;

  // Remove unused imports
  for (const importName of unusedImports) {
    const regex = new RegExp(`import \\{[^}]*\\b${importName}\\b[^}]*\\} from ['"'][^'"']+['"'];?\\s*\\n`, 'g');
    if (regex.test(content)) {
      content = content.replace(regex, '');
      modified = true;
    }
  }

  if (modified) {
    fs.writeFileSync(fullPath, content);
    console.log(`✅ Fixed unused imports in ${filePath}`);
    return true;
  }
  return false;
}

// Process all target files
let fixedCount = 0;
for (const file of targetFiles) {
  if (removeUnusedImports(file)) {
    fixedCount++;
  }
}

console.log(`\n📊 Fixed unused imports in ${fixedCount} files`);
