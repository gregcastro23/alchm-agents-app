        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Craft precise consciousness parameters for unique agent personality</CardDescription>
            </div>
          </div>
          <Button
            variant="outline"
      ...
      <CardContent className="space-y-6">
        ...
        <Tabs defaultValue="core" className="space-y-4">
          ...
          <TabsContent value="core" className="space-y-6">
            {renderParameterSlider('wisdom', 'Wisdom', <BookOpen className="w-4 h-4" />, 'Deep knowledge and understanding')}
            {renderParameterSlider('charisma', 'Charisma', <Star className="w-4 h-4" />, 'Social magnetism and influence')}
            {renderParameterSlider('intuition', 'Intuition', <Eye className="w-4 h-4" />, 'Spiritual sensitivity and insight')}
            {renderParameterSlider('analytical', 'Analytical', <Brain className="w-4 h-4" />, 'Logical reasoning and structure')}
            {renderParameterSlider('creativity', 'Creativity', <Sparkles className="w-4 h-4" />, 'Innovation and artistic expression')}
            {renderParameterSlider('empathy', 'Empathy', <Heart className="w-4 h-4" />, 'Emotional intelligence and compassion')}
            {renderParameterSlider('authority', 'Authority', <Crown className="w-4 h-4" />, 'Leadership and command presence')}
            {renderParameterSlider('mysticism', 'Mysticism', <Gem className="w-4 h-4" />, 'Connection to transcendent realms')}
            {renderParameterSlider('practicality', 'Practicality', <Settings className="w-4 h-4" />, 'Grounded, real-world effectiveness')}
            {renderParameterSlider('humor', 'Humor', <Zap className="w-4 h-4" />, 'Wit, playfulness, and levity')}
          </TabsContent>

          <TabsContent value="communication" className="space-y-6">
            {renderParameterSlider('formality', 'Formality', <Shield className="w-4 h-4" />, 'Formal vs casual communication style')}
            {renderParameterSlider('directness', 'Directness', <Target className="w-4 h-4" />, 'Blunt vs diplomatic approach')}
            {renderParameterSlider('eloquence', 'Eloquence', <MessageSquare className="w-4 h-4" />, 'Linguistic sophistication and artistry')}
            {renderParameterSlider('passion', 'Passion', <Heart className="w-4 h-4" />, 'Emotional intensity in expression')}
            {renderParameterSlider('patience', 'Patience', <Activity className="w-4 h-4" />, 'Tolerance and measured responses')}
          </TabsContent>

          <TabsContent value="modes" className="space-y-6">
            {renderParameterSlider('teacherMode', 'Teacher Mode', <BookOpen className="w-4 h-4" />, 'Instructional and educational focus')}
            {renderParameterSlider('counselorMode', 'Counselor Mode', <Heart className="w-4 h-4" />, 'Supportive and therapeutic approach')}
            {renderParameterSlider('visionaryMode', 'Visionary Mode', <Eye className="w-4 h-4" />, 'Future-focused and inspirational')}
            {renderParameterSlider('scholarMode', 'Scholar Mode', <BookOpen className="w-4 h-4" />, 'Research and academic orientation')}
            {renderParameterSlider('mysticMode', 'Mystic Mode', <Gem className="w-4 h-4" />, 'Spiritual and transcendent perspective')}
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>
