...
export default function MonicaChatInterface({
    ...
    try {
      const response = await fetch('/api/monica-agent', {
        ...
        body: JSON.stringify({
          ...
          quickProfile: messages.length <= 1 ? { goal: 'fast_onboarding', mood: 'open', topFocus: ['ai_personality'] } : null,
      ...
      if (data.response) {
        ...
        const errorMessage: MonicaChatMessage = {
          ...
          type: 'monica',
          content: data.response || "I'm having a moment of cosmic static, dear one. Let's try that again - I'm here for you! 💚",
          timestamp: new Date()
        };
      ...
      const errorMessage: MonicaChatMessage = {
        id: Date.now().toString() + '_error',
        type: 'monica',
        content: "Oh my, I seem to be having connection troubles. My Virgo rising wants everything to work perfectly! Please try again, and I'll be right here waiting to help you explore your cosmic nature. 🌸",
        timestamp: new Date()
      };
      setMessages(prev => [...prev, errorMessage]);
    } finally {
  ...
  const sendMessageStream = async () => {
    if (!inputValue.trim() || isLoading) return;
    const userMessage: MonicaChatMessage = { id: Date.now().toString(), type: 'user', content: inputValue, timestamp: new Date() };
    setMessages(prev => [...prev, userMessage]);
    const current = inputValue; setInputValue(''); setIsLoading(true);
    try {
      const resp = await fetch('/api/monica-agent/stream', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: current, conversationStage: messages.length <= 1 ? 'greeting' : 'teaching', preferredStyle: { temperature: 0.4 } }) })
      if (!resp.ok || !resp.body) { await sendMessage(); return; }
        ...
        for (const part of parts) {
          ...
          try {
            ...
            if (evt.type === 'envelope') {
              envelope = { suggestedPractices: evt.payload.suggestedPractices || [], nextStep: evt.payload.nextStep || '', followUps: evt.payload.followUps || [] };
            }
          } catch {}
        }
      }
      const monicaMsg: MonicaChatMessage = { id: Date.now().toString() + '_monica', type: 'monica', content: assembled || '...', timestamp: new Date(), envelope };
        ...
        <CardContent className="p-0 h-full">
          <ScrollArea className="h-full p-4" ref={scrollAreaRef}>
              ...
              {messages.map((message) => (
                ...
                >
                  <div className={`max-w-[80%] ${
                    ...
                    <div className={`${message.type === 'user' ? 'text-primary-foreground' : 'text-gray-800'}`}>
                      {message.content}
                    </div>
                    ...
                    {message.type === 'monica' && message.envelope && (
                      <div className="mt-3 space-y-2">
                        <div>
                          <div className="text-xs font-semibold text-green-700">Suggested Practices</div>
            ...
            {message.envelope.followUps?.length > 0 && (
                  ...
                  <Button key={i} size="sm" variant="outline" className="text-xs" onClick={() => setInputValue(q)}>
                                {q}
                              </Button>
                            ))}
                          </div>
        ...
        <CardContent className="p-4">
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
            <div className="flex items-center gap-1">
              ...
              <span>Monica: World-renowned tarot master & expert guide to the Planetary Agents system</span>
            </div>
          </div>
        </CardContent>
      </Card>
