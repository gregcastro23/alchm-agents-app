...
if (!apiKey) {
  console.warn('Warning: Neither ANTHROPIC_API_KEY nor OPENAI_API_KEY is set in environment variables');
}
...
export async function createClaudeMessage(
  ...
  return await anthropic.messages.create({
    ...
    system: system || "You are a helpful AI assistant specializing in astrological and alchemical wisdom.",
    messages
  });
}
...
export async function logToAnthropicStream(
  ...
  try {
    ...
    ], "You are a log processing agent. Your job is to receive logs, acknowledge them, and provide insights if needed. Be brief and efficient in your responses.", 'fast');
