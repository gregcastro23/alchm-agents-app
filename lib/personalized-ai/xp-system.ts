...
export function calculateXP(
  ...
  const baseXP = Math.round(
    (interactionQuality / 100) * XP_CONFIG.MAX_INTERACTION_XP
  );
  
  // Feedback bonus
  let feedbackBonus = 0;
  if (userFeedback && userFeedback.explicit) {
    const multiplier = XP_CONFIG.FEEDBACK_MULTIPLIERS[userFeedback.rating as keyof typeof XP_CONFIG.FEEDBACK_MULTIPLIERS] || 1.0;
    feedbackBonus = Math.round(baseXP * (multiplier - 1));
  }
  
  // Training focus bonus
  const focusBonus = trainingFocus 
    ? Math.round(baseXP * (XP_CONFIG.FOCUS_MULTIPLIER - 1))
    : 0;
  ...
  const astrologicalBonus = Math.round(
    baseXP * XP_CONFIG.ASTROLOGICAL_BONUS[astrologicalInfluence.toUpperCase() as keyof typeof XP_CONFIG.ASTROLOGICAL_BONUS]
  );
...
export function getXPBonusRecommendations(
  ...
  if (currentStreak === 0) {
    ...
    recommendations.push(`Keep your ${currentStreak} day streak going! Reach 7 days for 1.3x XP bonus.`);
  ...
  if (!lastTrainingFocus && lowestScoreCategory) {
    recommendations.push(`Focus training on ${lowestScoreCategory.replace('_', ' ')} for 2x XP bonus!`);
  }
