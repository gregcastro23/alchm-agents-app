...
import { analyzeAllAspectDynamics, findNearestApplyingAspect, type AspectsSamplingOptions } from "@/lib/aspects-sampling";
...
export async function GET(request: Request) {
  try {
    ...
    console.log("API: Aspects dynamics endpoint called", {
      lat, lon, dateParam, window, includeKinetics, mode
    });
    ...
    if (mode === 'nearest') {
      ...
      return NextResponse.json({
        ...
        metadata: {
          planetsAnalyzed: targetPlanets || ['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn'],
          window,
          includeKinetics,
    ...
    const responseData = {
      ...
      aspects: aspectsResult.aspects.map(aspect => ({
        ...
        kineticInfluence: aspect.kineticInfluence ? Math.round(aspect.kineticInfluence * 1000) / 1000 : undefined
      })),
      metadata: {
  ...
  try {
    ...
    if (planets) {
      if (typeof planets === 'string') {
        targetPlanets = planets.split(',').map(p => p.trim()).filter(p => p.length > 0);
