...
function wrapApiEndpoint(filePath) {
  try {
    ...
    if (content.includes('withErrorHandling') || content.includes('AgentErrorHandler.executeWithFallback')) {
...
function wrapFunction(content, method, filePath) {
  ...
  const endpointPath = path.relative('app/api', filePath).replace('/route.ts', '').replace(/\//g, '_');
  const wrappedBody = `
