...
import { 
  PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, Radar
} from 'recharts';
import {
  Star,
  Brain,
...
export function CharacterVectorDashboard({
  ...
  const [activeTab, setActiveTab] = useState<'vectors' | 'interactions' | 'learning' | 'preferences'>('vectors');
  const [selectedSign, setSelectedSign] = useState<string | null>(null);
  const [learningRecommendations, setLearningRecommendations] = useState<any>(null);

  // Calculate user's character profile
  const userCharacterProfile = CharacterVectorCalculator.generateChartCharacterProfile(userProfile.planetaryPlacements);
  
  // Calculate current moment interaction if available
  const currentMomentProfile = currentMomentChart ? 
    CharacterVectorCalculator.generateChartCharacterProfile(currentMomentChart.planetaryPlacements) : null;
  
  const currentMomentInteraction = currentMomentProfile ? 
    CharacterVectorCalculator.analyzeChartInteraction(userCharacterProfile, currentMomentProfile) : null;
  ...
  const elementalData = Object.entries(userCharacterProfile.elemental_distribution)
  ...
  const modalData = Object.entries(userCharacterProfile.modal_distribution)
    .map(([mode, percentage]) => ({
  ...
  const interactionRadarData = [
    { preference: 'Visual', value: userCharacterProfile.interaction_style_preferences.learning_modalities.visual },
    { preference: 'Auditory', value: userCharacterProfile.interaction_style_preferences.learning_modalities.auditory },
    { preference: 'Kinesthetic', value: userCharacterProfile.interaction_style_preferences.learning_modalities.kinesthetic },
    { preference: 'Reading', value: userCharacterProfile.interaction_style_preferences.learning_modalities.reading },
    { preference: 'Direct', value: userCharacterProfile.interaction_style_preferences.communication_preferences.direct_vs_diplomatic },
    { preference: 'Fast-Paced', value: userCharacterProfile.interaction_style_preferences.communication_preferences.fast_vs_slow }
  ];
  ...
  function getElementColor(element: string): string {
    const colors: Record<string, string> = {
      fire: '#FF6B6B', earth: '#4ECDC4', air: '#45B7D1', water: '#96CEB4'
    };
  ...
  function getModalColor(mode: string): string {
    const colors: Record<string, string> = {
      cardinal: '#FF6B6B', fixed: '#4ECDC4', mutable: '#45B7D1'
    };
  ...
  function getModalIcon(mode: string) {
    switch (mode.toLowerCase()) {
      case 'cardinal': return <Triangle className="h-4 w-4 text-red-500" />;
      case 'fixed': return <Square className="h-4 w-4 text-blue-500" />;
      case 'mutable': return <Circle className="h-4 w-4 text-green-500" />;
      default: return <Star className="h-4 w-4" />;
    }
  }
      ...
      <Card className="bg-gradient-to-r from-purple-50 to-indigo-50 border-purple-200">
        <CardHeader>
          <CardTitle className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">{userProfile.name}&apos;s Character Vector Profile</h1>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">{userCharacterProfile.dominant_signs.length}</div>
              <div className="text-xs text-muted-foreground">Dominant Signs</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">{userCharacterProfile.absent_signs.length}</div>
              <div className="text-xs text-muted-foreground">Absent Signs</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {Object.values(userCharacterProfile.elemental_distribution).findIndex(v => v === Math.max(...Object.values(userCharacterProfile.elemental_distribution))) + 1}
              </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        cy="50%"
                        labelLine={false}
                        label={({sign, percentage}) => percentage > 5 ? `${sign} ${percentage}%` : ''}
                        outerRadius={80}
                        fill="#8884d8"
                        dataKey="percentage"
                      >
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {userCharacterProfile.dominant_signs.map((sign) => {
                      const percentage = userCharacterProfile.sign_vectors[sign as keyof SignCharacterVector];
                      return (
                        <Badge 
                          key={sign} 
                          variant="outline" 
              ...
              <CardContent className="space-y-4">
                ...
                {currentMomentInteraction.missing_sign_dynamics.length > 0 && (
                  <div className="space-y-2">
                    <h4 className="font-medium text-sm">Learning Opportunities from Current Energies:</h4>
                ...
                <div className="p-3 bg-blue-50 rounded-lg">
                  <h4 className="font-medium text-sm text-blue-800 mb-1">Recommended Interaction Style</h4>
                  <p className="text-xs text-blue-700">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Based on your astrological character vectors
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-80">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Personalized based on your character profile
                </CardDescription>
              </CardHeader>
              <CardContent>
                {learningRecommendations ? (
                  <div className="space-y-3">
                    {learningRecommendations.recommended_modes.map((mode: string, index: number) => (
                      <Button
                        key={index}
                        variant="outline"
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {learningRecommendations.session_structure.main_content.segments} segments with {' '}
                      {learningRecommendations.session_structure.main_content.length_per_segment === 'short_bursts' 
            ...
            <CardContent>
              <div className="space-y-6">
                <div className="text-center text-muted-foreground">
                  ...
                  <p className="text-sm">
                    This would include settings for training modes, session preferences, 
                    interaction styles, and personalization options
                  </p>
                </div>
              </div>
            </CardContent>
