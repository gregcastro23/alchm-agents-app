...
describe('PlanetaryHoursService', () => {
    ...
    describe('getCurrentPlanetaryHour', () => {
        it('should return current planetary hour information', async () => {
            ...
            expect(['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn']).toContain(result.planet);
        ...
        it('should handle different times of day correctly', async () => {
            ...
            const midnightResult = await planetaryHoursService.getCurrentPlanetaryHour(midnightTime, testLocation);
    ...
    describe('getForecast', () => {
        it('should return forecast for date range', async () => {
            ...
            forecast.forEach(entry => {
                ...
                expect(['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn']).toContain(entry.planetaryHour.planet);
            });
        });
        it('should handle different intervals correctly', async () => {
            const startDate = new Date();
            const endDate = new Date(startDate.getTime() + 6 * 60 * 60 * 1000); // 6 hours later
            const forecast60 = await planetaryHoursService.getForecast(startDate, endDate, testLocation, 60);
            const forecast120 = await planetaryHoursService.getForecast(startDate, endDate, testLocation, 120);
    ...
    describe('getOptimalTimes', () => {
        it('should return optimal times for specific planet', async () => {
            ...
            const optimalTimes = await planetaryHoursService.getOptimalTimes(testDate, testLocation, targetPlanet);
        ...
        it('should return empty array for invalid planet', async () => {
            ...
            const optimalTimes = await planetaryHoursService.getOptimalTimes(testDate, testLocation, invalidPlanet);
            expect(Array.isArray(optimalTimes)).toBe(true);
    ...
    describe('Edge cases', () => {
        it('should handle extreme latitudes', async () => {
            ...
            const arcticResult = await planetaryHoursService.getCurrentPlanetaryHour(new Date(), arcticLocation);
            const antarcticResult = await planetaryHoursService.getCurrentPlanetaryHour(new Date(), antarcticLocation);
            expect(arcticResult).toHaveProperty('planet');
            expect(antarcticResult).toHaveProperty('planet');
            expect(['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn']).toContain(arcticResult.planet);
            expect(['Sun', 'Moon', 'Mercury', 'Venus', 'Mars', 'Jupiter', 'Saturn']).toContain(antarcticResult.planet);
        });
        it('should handle date boundaries correctly', async () => {
            ...
            const startResult = await planetaryHoursService.getCurrentPlanetaryHour(startOfDay, testLocation);
            expect(endResult).toHaveProperty('planet');
            expect(startResult).toHaveProperty('planet');
        });
    });
});
//# sourceMappingURL=planetary-hours.test.js.map