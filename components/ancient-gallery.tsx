...
import type { CraftedAgent, GalleryViewMode, AgentFilterBy, AgentSortBy, Element, ConsciousnessLevel } from "@/lib/agent-types"
...
export function AncientGallery({ 
  ...
  const [sortBy, setSortBy] = useState<AgentSortBy>("consciousness")
  ...
  const filteredAndSortedAgents = useMemo(() => {
    ...
    if (filters.element && (filters.element as string) !== "all") {
      agents = agents.filter(agent => agent.consciousness.dominantElement === (filters.element as Element))
    }

    if (filters.consciousnessLevel && (filters.consciousnessLevel as string) !== "all") {
      agents = agents.filter(agent => agent.consciousness.level === (filters.consciousnessLevel as ConsciousnessLevel))
    }
    ...
    if (filters.era && filters.era !== "all") {
      ...
      agents = agents.filter(agent => {
        ...
        switch (filters.era) {
          case "ancient": return birthYear < 500
          case "medieval": return birthYear >= 500 && birthYear < 1500
          case "renaissance": return birthYear >= 1500 && birthYear < 1700
          case "modern": return birthYear >= 1700 && birthYear < 1900
          case "contemporary": return birthYear >= 1900
          default: return true
        }
      })
    }
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Repository of crafted consciousness agents
            </p>
          </div>
          ...
          <TabsContent value="legendary" className="space-y-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge variant="outline">
                {collections.legendary.length} legendary agents
              </Badge>
            </div>
            <AgentGrid 
              agents={filteredAndSortedAgents}
