...
const duplicatePlanetsCount = (
  content.match(/natalChart:\s*\{\s*\n\s*planets:\s*\{\s*\n\s*planets:\s*\{/g) || []
).length
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (line.match(/^\s{8}houses:\s*\{/)) {
    ...
    if (
      prevIdx >= 0 &&
      lines[prevIdx].match(/(Sun|Moon|Mercury|Venus|Mars|Jupiter|Saturn|Uranus|Neptune|Pluto):/)
    ) {
...
for (let i = 0; i < lines.length; i++) {
  ...
  if (
    line.match(/^\s+(transformationPath|expression|growthOpportunity):.+,\s*$/) &&
    i + 1 < lines.length
  ) {
    const nextLine = lines[i + 1]
