...
function calculatePlanetPosition(planet, birthDate, referenceDate = new Date(2024, 0, 1)) {
    ...
    if (newAbsoluteDegrees < 0)
        newAbsoluteDegrees += 360;
...
function calculateAscendant(birthInfo) {
    ...
    let ascendantLongitude = Math.atan2(-Math.cos(RAMC), Math.sin(RAMC) * cosObliquity + tanLat * Math.sin(obliquity)) * 180 / Math.PI;
