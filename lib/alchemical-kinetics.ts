...
export function getPlanetaryInertiaModifier(hour: PlanetaryHour): number {
  ...
  return hour === 'Saturn' ? 1.10 : 1.0;
}

export function getSolarAmplification(hour?: PlanetaryHour): number {
  return hour === 'Sun' ? 1.30 : 1.0; // +30% typical during Sun hours
}
...
export function computeElementalVelocity(
  ...
  for (let i = 0; i < samples.length; i++) {
    ...
    const magnitude = Math.sqrt(
      rawV.Fire * rawV.Fire + rawV.Water * rawV.Water + rawV.Air * rawV.Air + rawV.Earth * rawV.Earth
    );
...
export function computeMetricVelocity(
  ...
  if (!samples || samples.length === 0) return [];
  const out: Array<{ t: Date; dvdt: MetricVector; thermalDirection: 'heating' | 'cooling' | 'stable' }> = [];
  for (let i = 0; i < samples.length; i++) {
    const current = samples[i];
    const previous = i > 0 ? samples[i - 1] : undefined;
    const dt = previous ? (current.t.getTime() - previous.t.getTime()) / 3600000 : 0; // hours
...
export function computeElementalMomentum(
  ...
  if (!samples || samples.length === 0) return [];
  const out: Array<{ t: Date; p: ElementVector; magnitude: number; momentumType: 'building' | 'sustained' | 'dissipating' }> = [];
  for (let i = 0; i < samples.length; i++) {
    const current = samples[i];
    const previous = i > 0 ? samples[i - 1] : undefined;
    ...
    const magnitude = Math.sqrt(p.Fire * p.Fire + p.Water * p.Water + p.Air * p.Air + p.Earth * p.Earth);
...
export function computeInertia(
  input: { matter: number; earth: number; substance: number; planetaryHour?: PlanetaryHour }
): number {
...
export function validateKineticResults(
  ...
  // Range checks
  const vMax = kinetics.elementalVelocity?.reduce((m: number, s: any) => Math.max(m, s.magnitude || 0), 0) ?? 0;
  const pMax = kinetics.elementalMomentum?.reduce((m: number, s: any) => Math.max(m, s.magnitude || 0), 0) ?? 0;
  const powMax = kinetics.power?.reduce((m: number, s: any) => Math.max(m, s.power || 0), 0) ?? 0;
  if (vMax > expectedRanges.velocityMax) warnings.push(`Velocity magnitude exceeds expected max (${vMax.toFixed(3)} > ${expectedRanges.velocityMax}).`);
  if (pMax > expectedRanges.momentumMax) warnings.push(`Momentum magnitude exceeds expected max (${pMax.toFixed(3)} > ${expectedRanges.momentumMax}).`);
  if (powMax > expectedRanges.powerMax) warnings.push(`Power exceeds expected max (${powMax.toFixed(3)} > ${expectedRanges.powerMax}).`);

  // Timing expectations (heuristic counts over series)
  const countByHour: Record<string, { fireLead: number; total: number; powerSum: number; powerCount: number }> = {};
  (kinetics.elementalVelocity ?? []).forEach((s: any) => {
    const hour = s.planetaryHour ?? s.hour ?? 'unknown';
    if (!countByHour[hour]) countByHour[hour] = { fireLead: 0, total: 0, powerSum: 0, powerCount: 0 };
    // Fire leads when v.Fire is the max among elements
    const v = s.v as ElementVector;
    const maxEl = (['Fire', 'Water', 'Air', 'Earth'] as ElementKey[]).reduce((a, b) => (v[a] >= v[b] ? a : (b as ElementKey)));
  ...
  if (overallPowerAvg > 0) {
    ...
    if (ratio < 1.2) warnings.push('Power shows weak solar amplification (<20% over baseline during Sun hours).');
  }
