...
export async function GET(request: NextRequest) {
  try {
    ...
    const avgResponseTime = agentStats.reduce((sum, stat) => 
      sum + (stat._avg.responseTime || 0), 0) / (agentStats.length || 1)
    ...
    const metrics = {
      ...
      performanceMetrics: {
        systemHealth: activeUsers > 100 ? 'excellent' : activeUsers > 50 ? 'good' : activeUsers > 10 ? 'fair' : 'poor',
        uptime: 99.95, // Would come from monitoring service
        responseTime: avgResponseTime || 250,
        throughput: Math.floor(recentConversations / ((now.getTime() - startTime.getTime()) / 60000)), // per minute
        errorRate: 0.015,
        alerts: [],
      },
    }
