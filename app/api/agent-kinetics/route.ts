...
function calculateAgentKinetics(
  ...
  if (alignmentScore > 0.85) {
    alignmentLevel = 'peak'
    alignmentDescription = 'Optimal cosmic alignment - perfect for profound interactions and breakthrough insights'
  } else if (alignmentScore > 0.7) {
    alignmentLevel = 'elevated'
    alignmentDescription = 'Strong cosmic support - excellent time for meaningful dialogue and exploration'
    ...
    alignmentLevel = 'low'
    alignmentDescription = 'Gentle approach recommended - best for simple questions and basic guidance'
  } else {
...
export async function POST(req: Request) {
  try {
    ...
    const sortedMetrics = agentMetrics.sort((a, b) => b.cosmicAlignment.score - a.cosmicAlignment.score)

    // Calculate group dynamics
    const groupSummary = {
      totalAgents: agentMetrics.length,
      averageAlignment: agentMetrics.reduce((sum, m) => sum + m.cosmicAlignment.score, 0) / agentMetrics.length,
      momentumDistribution: agentMetrics.reduce((acc, m) => {
