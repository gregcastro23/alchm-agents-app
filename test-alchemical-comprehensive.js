...
async function testAlchemicalTraining() {
  ...
  try {
    ...
    if (data.success && data.data) {
      ...
      if (result.monicaConstant && result.monicaConstant.average !== undefined) {
        ...
        log(`  Interpretation: ${result.monicaConstant.interpretation?.substring(0, 100)}...`, 'blue');
        passedTests++;
      } else {
  ...
  try {
    ...
    if (data.success && data.data) {
      ...
      if (result.peaks && result.averages) {
        ...
        log(`  Spirit peaks at ${result.peaks.spirit.hour}:00 (${result.peaks.spirit.ruler})`, 'cyan');
        log(`  Essence peaks at ${result.peaks.essence.hour}:00 (${result.peaks.essence.ruler})`, 'cyan');
  ...
  try {
    const response = await fetch(`${baseUrl}/api/monica-agent`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        message: 'Monica, calculate the Monica Constant for Spirit: 5, Essence: 7, Matter: 3, Substance: 2',
        includeAlchm: true
      })
    });
    const data = await response.json();
