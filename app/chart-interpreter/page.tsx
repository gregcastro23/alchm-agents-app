...
import EnhancedChartDisplay from "@/components/enhanced-chart-display"
import QuickChartInput from "@/components/quick-chart-input"
import SavedChartsManager from "@/components/saved-charts-manager"
import AspectGrid from "@/components/aspect-grid"
...
export default function ChartInterpreterPage() {
  ...
  const { planetaryPositions, alchmQuantities, monicaConstant, lastUpdated } = usePlanetaryPositions({
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.responses) {
        ...
        setInterpretation(fallbackData.interpretation || `Error: ${data.error || "Failed to interpret chart"}`)
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (chartData.chartType === 'current' && planetaryPositions.length > 0) {
      const currentPlanets = planetaryPositions.reduce((acc, pos) => {
      ...
      {viewMode === 'input' ? (
        ...
        <div className="space-y-6">
          ...
          <div className="max-w-4xl mx-auto">
            <QuickChartInput 
              onChartParsed={handleQuickInputParsed}
              className=""
            />
          </div>
          ...
          <Card className="max-w-4xl mx-auto">
            ...
            <CardContent>
              <Tabs 
                ...
                onValueChange={(value) => setChartData(prev => ({ ...prev, chartType: value as ChartType }))}
                className="w-full"
              >
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                ...
                {chartData.chartType === 'birth' ? 'Birth Details' : chartData.chartType === 'current' ? 'Current Moment' : 'Synastry Details'}
              </CardTitle>
              <CardDescription>
              ...
              {chartData.chartType === 'current' ? (
                <div className="space-y-4">
                  <Alert>
                    ...
                    <AlertDescription>
                      Current moment chart automatically populated with live planetary positions.
                      Monica Constant: {monicaConstant.toFixed(3)} | Spirit: {alchmQuantities.spirit.toFixed(2)}
                    </AlertDescription>
                  </Alert>
                  <Button 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-200px)]">
          ...
          <div className="space-y-4">
            ...
            <EnhancedChartDisplay 
              ...
              chartName={chartData.name || `${chartData.chartType.charAt(0).toUpperCase() + chartData.chartType.slice(1)} Chart`}
                      ...
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mb-4">
                            Click below to get AI-enhanced interpretation from the planetary council
                          </p>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="flex items-center justify-between p-2 bg-muted rounded">
                          ...
                          <div className="text-right">
                            <div className="font-mono text-sm">{alchmQuantities.spirit.toFixed(2)}</div>
                            <div className="w-16 h-1 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-purple-500 transition-all duration-500"
                                style={{ width: `${Math.min(100, (alchmQuantities.spirit / 20) * 100)}%` }}
                              />
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center justify-between p-2 bg-muted rounded">
                          ...
                          <div className="text-right">
                            <div className="font-mono text-sm">{alchmQuantities.essence.toFixed(2)}</div>
                            <div className="w-16 h-1 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-blue-500 transition-all duration-500"
                                style={{ width: `${Math.min(100, (alchmQuantities.essence / 20) * 100)}%` }}
                              />
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center justify-between p-2 bg-muted rounded">
                          ...
                          <div className="text-right">
                            <div className="font-mono text-sm">{alchmQuantities.matter.toFixed(2)}</div>
                            <div className="w-16 h-1 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-green-500 transition-all duration-500"
                                style={{ width: `${Math.min(100, (alchmQuantities.matter / 20) * 100)}%` }}
                              />
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center justify-between p-2 bg-muted rounded">
                          ...
                          <div className="text-right">
                            <div className="font-mono text-sm">{alchmQuantities.substance.toFixed(2)}</div>
                            <div className="w-16 h-1 bg-gray-200 rounded-full overflow-hidden">
                              <div 
                                className="h-full bg-orange-500 transition-all duration-500"
                                style={{ width: `${Math.min(100, (alchmQuantities.substance / 20) * 100)}%` }}
                              />
                            </div>
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-mono">{alchmQuantities.Reactivity.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-xs text-muted-foreground">
                            {monicaConstant > 4.236 ? 'Elevated' : 
                             monicaConstant > 3.618 ? 'Active' : 
                             monicaConstant > 2.618 ? 'Awakening' : 'Dormant'}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="text-xs text-muted-foreground space-y-1">
                      <p>
                        <strong>Balance:</strong> {alchmQuantities.spirit > alchmQuantities.matter ? 'Spiritually oriented' : 'Materially grounded'} •
                        {alchmQuantities.essence > alchmQuantities.substance ? ' Essence-driven' : ' Substance-focused'}
                      </p>
                      <p>
                        <strong>Primary Energy:</strong> {alchmQuantities.Heat > 50 ? 'High heat (transformative)' : 'Stable heat (consistent)'} •
                        {alchmQuantities.Reactivity > 30 ? ' Highly reactive' : ' Balanced reactivity'}
                      </p>
                    </div>
                  </CardContent>
                </Card>
