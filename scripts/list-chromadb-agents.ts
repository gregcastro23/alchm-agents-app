...
async function listAgents() {
  ...
  try {
    ...
    sortedAgents.forEach(([id, data], idx) => {
      console.log(`${(idx + 1).toString().padStart(2)}. ${data.name.padEnd(30)} | Chunks: ${data.chunks.toString().padStart(2)} | Era: ${data.era.padEnd(15)} | Level: ${data.level}`);
    });
    ...
    console.log(`\n📊 Agents by Era:`);
    Array.from(byEra.entries()).sort((a, b) => b[1] - a[1]).forEach(([era, count]) => {
