...
function printStatus(status, message) {
  const color = colors[status === 'SUCCESS' ? 'green' : status === 'ERROR' ? 'red' : status === 'WARNING' ? 'yellow' : 'blue'];
...
if (GALILEO_API_KEY) {
  ...
  if (GALILEO_API_KEY.startsWith('gal_') || GALILEO_API_KEY.length > 20) {
    ...
    printStatus('WARNING', 'API Key format may be invalid (should start with "gal_" or be a long string)');
  }
} else {
...
if (GALILEO_PROJECT) {
  ...
  const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(GALILEO_PROJECT);
  if (isUUID) {
...
if (GALILEO_LOG_STREAM) {
  ...
  // Check if it's a UUID or string
  const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(GALILEO_LOG_STREAM);
  if (isUUID) {
