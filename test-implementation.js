...
const SAMPLE_SURVEY_RESPONSES = [
  ...
  },
  {
    ...
    value: ['Writing and storytelling', 'Problem-solving and innovation'],
  },
  ...
  { questionId: 'ai_challenge_level', value: 'Give me moderate intellectual challenges' },
  {
    ...
    value: ['Philosophy and meaning', 'Psychology and human behavior', 'Personal development'],
  },
]
...
async function testTypeDefinitions() {
  ...
  try {
    ...
    recordTest(
      'Survey questions file contains questions array',
      content.includes('CONSCIOUSNESS_SURVEY_QUESTIONS')
    )
...
async function testSurveyProcessing() {
  ...
  try {
    ...
    recordTest(
      'Survey processor contains main function',
      content.includes('processSurveyResponses')
    )
    recordTest(
      'Survey processor has writing style analysis',
      content.includes('analyzeWritingStyle')
    )
    recordTest(
      'Survey processor builds consciousness profile',
      content.includes('buildConsciousnessProfile')
    )
    recordTest(
      'Survey processor generates insights',
      content.includes('generatePersonalityInsights')
    )
...
async function testXPSystem() {
  ...
  try {
    ...
    if (fs.existsSync(xpPath)) {
      ...
      recordTest(
        'XP system handles bonuses',
        xpContent.includes('bonus') || xpContent.includes('multiplier')
      )
    } else {
    ...
    if (fs.existsSync(levelPath)) {
      ...
      recordTest(
        'Level system contains calculation function',
        levelContent.includes('calculateLevel')
      )
      recordTest(
        'Level system has tier structure',
        levelContent.includes('tier') || levelContent.includes('level')
      )
    } else {
...
async function testAchievementSystem() {
  ...
  try {
    ...
    if (fs.existsSync(filePath)) {
      const content = fs.readFileSync(filePath, 'utf8')
      recordTest(
        'Achievement system contains definitions',
        content.includes('ACHIEVEMENT_DEFINITIONS') || content.includes('achievements')
      )
      recordTest('Achievement system has checking function', content.includes('checkAchievements'))
      recordTest(
        'Achievement system has different types',
        content.includes('first_words') || content.includes('achievement')
      )
      recordTest(
        'Achievement system handles XP rewards',
        content.includes('xp') || content.includes('XP')
      )
    } else {
...
async function testDualChartSystem() {
  ...
  try {
    ...
    if (fs.existsSync(filePath)) {
      const content = fs.readFileSync(filePath, 'utf8')
      recordTest(
        'Dual chart system contains creation function',
        content.includes('createDualChartSystem') || content.includes('DualChart')
      )
      recordTest(
        'Dual chart system handles birth chart',
        content.includes('birth') || content.includes('Birth')
      )
      recordTest(
        'Dual chart system handles current moment',
        content.includes('current') || content.includes('moment')
      )
      recordTest(
        'Dual chart system handles transits',
        content.includes('transit') || content.includes('Transit')
      )
    } else {
