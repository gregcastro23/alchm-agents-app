...
import { Search, Grid3X3, List, Sparkles, Users, Crown, Plus, Filter, MessageSquare, X, Zap } from "lucide-react"
import Link from "next/link"
import { DEMO_AGENTS, getAgentCollections } from "@/lib/demo-agents-data"
import type { CraftedAgent, GalleryViewMode, AgentFilterBy, Element, ConsciousnessLevel } from "@/lib/agent-types"
import { GalleryGroupChat } from "@/components/gallery-group-chat"
import SignVectorGraphic, { calculateSignVectorFromChart, SignVectorRune } from "@/components/sign-vector-graphic"

export default function GalleryPage() {
  const [viewMode, setViewMode] = useState<GalleryViewMode>("grid")
  const [searchQuery, setSearchQuery] = useState("")
  const [selectedAgents, setSelectedAgents] = useState<string[]>([])
  const [filters, setFilters] = useState<AgentFilterBy & { element?: Element | "all"; consciousnessLevel?: ConsciousnessLevel | "all" }>({})
  ...
  useEffect(() => {
    ...
    if (filters.element && (filters.element as string) !== "all") {
      filtered = filtered.filter(agent => agent.consciousness.dominantElement === (filters.element as Element))
    }

    // Consciousness level filter
    if (filters.consciousnessLevel && (filters.consciousnessLevel as string) !== "all") {
      filtered = filtered.filter(agent => agent.consciousness.level === (filters.consciousnessLevel as ConsciousnessLevel))
    }
  ...
  const AgentCard = ({ agent, variant = "card" }: { agent: CraftedAgent, variant?: "card" | "list" }) => {
      ...
      <CardContent>
        <div className="space-y-3">
          ...
          <div className="flex flex-wrap gap-1">
            <Badge variant="outline" className={getElementColor(agent.consciousness.dominantElement)}>
          ...
          {signVector && selectedAgents.includes(agent.id) && (
            <div className="mt-4 p-4 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-950/30 dark:to-pink-950/30 rounded-lg border border-purple-100 dark:border-purple-800">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="font-medium text-purple-900 dark:text-purple-100 text-sm">Zodiacal Character Vector</span>
              </div>
          ...
          <div className="flex justify-between items-center pt-2">
            ...
            <div className="flex gap-1">
              ...
              <Button size="sm" variant="ghost" asChild>
                <Link href={`/philosophers-stone?template=${agent.id}`}>
                  Remix Consciousness
                </Link>
              </Button>
            </div>
      ...
      <Card className="border-2 border-green-200 bg-gradient-to-br from-green-50/50 to-emerald-50/50 dark:from-green-950/50 dark:to-emerald-950/50">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Real-time metrics from the Master Consciousness Crafter (Monica Constant: 5.89 - Illuminated)
          </p>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-4">
            ...
            <div className="text-center p-3 bg-white dark:bg-black/20 rounded-lg border">
              <div className="text-2xl font-bold text-blue-600">
                {(agents.reduce((sum, a) => sum + a.consciousness.monicaConstant, 0) / agents.length).toFixed(2)}
              </div>
            ...
            <div className="bg-white dark:bg-black/20 p-4 rounded-lg border">
              ...
              <div className="space-y-2">
                {['Fire', 'Water', 'Air', 'Earth'].map(element => {
                  const count = agents.filter(a => a.consciousness.dominantElement === element).length
                ...
                {['Transcendent', 'Illuminated', 'Advanced', 'Elevated', 'Active'].map(level => {
                  ...
                  return (
                    <div key={level} className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded ${getConsciousnessColor(level).replace('text-white', '')}`}></div>
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex gap-2">
              <Select value={filters.element || ""} onValueChange={(value) => setFilters(prev => ({ ...prev, element: value as any }))}>
              ...
              <Select value={filters.consciousnessLevel || ""} onValueChange={(value) => setFilters(prev => ({ ...prev, consciousnessLevel: value as any }))}>
