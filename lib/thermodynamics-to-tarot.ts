...
const TAROT_CARDS = {
  wands: [
    ...
    { name: 'Nine of Wands', number: 9, meaning: 'Resilience, courage, persistence, test of faith' },
    ...
    { name: 'Knight of Wands', court: 'knight', meaning: 'Energy, passion, adventure, impulsiveness' },
  ...
  cups: [
    ...
    { name: 'Queen of Cups', court: 'queen', meaning: 'Emotional security, intuitive, compassionate' },
  ...
  swords: [
    ...
    { name: 'Seven of Swords', number: 7, meaning: 'Betrayal, deception, getting away with something' },
    { name: 'Eight of Swords', number: 8, meaning: 'Imprisonment, restriction, self-limiting beliefs' },
    ...
    { name: 'Queen of Swords', court: 'queen', meaning: 'Independence, unbiased judgement, clear boundaries' },
    { name: 'King of Swords', court: 'king', meaning: 'Mental clarity, intellectual power, authority' }
  ],
  pentacles: [
    ...
    { name: 'King of Pentacles', court: 'king', meaning: 'Financial success, security, leadership' }
  ]
};
...
const MAJOR_ARCANA_CARDS = [
  ...
  { name: 'The High Priestess', number: 2, meaning: 'Intuition, sacred knowledge, divine feminine' },
...
function validateThermodynamicInput(metrics: ThermodynamicMetrics): ThermodynamicMetrics {
  return {
    ...
    reactivity: Math.max(0, Math.min(100, typeof metrics.reactivity === 'number' ? metrics.reactivity : 0)),
...
function selectCardRecommendations(
  ...
  topSuits.forEach((suitEmphasis, index) => {
    ...
    if (modalityEmphasis === 'cardinal' && (reactivity > 60 || energy > 60)) {
      ...
      const actionCards = suitCards.filter(card => 
        card.number === 1 || card.court === 'page' || card.court === 'knight' || 
      ...
      const stableCards = suitCards.filter(card => 
        [4, 6, 7, 9, 10].includes(card.number || 0) || card.court === 'queen' || card.court === 'king'
      );
      selectedCard = stableCards[Math.floor(stableCards.length * ((100 - entropy) / 100))] || stableCards[0];
      relevance += 0.1;
    } else {
      // Mutable or mixed - prefer adaptable cards
      const adaptableCards = suitCards.filter(card => 
        [2, 3, 5, 6, 7].includes(card.number || 0) || card.court === 'page' || card.court === 'knight'
      );
      selectedCard = adaptableCards[Math.floor(adaptableCards.length * (entropy / 100))] || adaptableCards[0];
    }
  ...
  if (totalEnergy > 150) {
    const highEnergyCards = MAJOR_ARCANA_CARDS.filter(card => 
      [1, 7, 8, 10, 13, 16, 19, 20].includes(card.number || 0) // High-action major arcana
    );
...
export function getTarotRecommendations(metrics: ThermodynamicMetrics): TarotRecommendationResult {
  ...
  const cardRecommendations = selectCardRecommendations(suitEmphases, validMetrics, modalityEmphasis);
