 ...
 * Calculate communication style metrics
 */
function calculateCommunicationStyle(birthChart: BirthChartData): BasePersonality['communicationStyle'] {
  let formality = 50;
  let verbosity = 50;
  let emotiveness = 50;
  let directness = 50;
 ...
 */
function analyzePlanetaryInfluences(birthChart: BirthChartData): BasePersonality['planetaryInfluences'] {
...
function calculatePlanetStrength(planet: string, sign: string, birthChart: BirthChartData): number {
  ...
  const aspectCount = birthChart.aspects?.filter(a => 
...
export function generateInitialTrainingScores(birthChart: BirthChartData): TrainingScores {
  ...
  return {
    communication_style: Math.min(70, baseScore + (elements.air * 0.2) + (mercurySign === 'Gemini' ? 10 : 0)),
    knowledge_depth: Math.min(70, baseScore + (elements.earth * 0.15) + (birthChart.alchemicalData.Matter * 0.1)),
    emotional_intelligence: Math.min(70, baseScore + (elements.water * 0.2) + (moonSign === 'Cancer' ? 10 : 0)),
    creativity: Math.min(70, baseScore + (elements.fire * 0.15) + (birthChart.alchemicalData.Spirit * 0.1)),
    memory_integration: Math.min(70, baseScore + (elements.earth * 0.1) + (birthChart.alchemicalData.Substance * 0.1)),
