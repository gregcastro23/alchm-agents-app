...
export async function POST(req: Request) {
  ...
  try {
    ...
    const momentum: 'building' | 'sustained' | 'peak' | 'waning' =
    ...
    const momentumModifier = ({ building: 1.1, sustained: 1.2, peak: 1.5, waning: 0.9 } as const)[
      momentum
    ]
    ...
    const planetaryRarity =
      ({ Sun: 0.2, Moon: 0.15, Jupiter: 0.25, Venus: 0.1 } as Record<string, number>)[
        planetaryHour
      ] || 0.05
    const seasonalRarity =
      ({ Spring: 0.1, Summer: 0.05, Autumn: 0.15, Winter: 0.2 } as Record<string, number>)[
        seasonal
      ] || 0
    const totalRarity = Math.min(
      1.0,
      baseNFTRarity + powerRarityBoost + planetaryRarity + seasonalRarity
    )
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        accumulationForecast:
          momentum === 'peak'
            ? 'PEAK PERIOD - Maximum generation active NOW'
            : momentum === 'sustained'
              ? 'Stable accumulation period - consistent generation expected'
              : momentum === 'building'
                ? 'Next 2-4 hours optimal for accumulation (momentum building)'
                : 'Generation slowing - consider waiting for next cycle',
        solarAmplification: planetaryHour === 'Sun' ? 1.3 : 1.0,
        seasonalModifier:
          ({ Spring: 1.1, Summer: 1.2, Autumn: 0.95, Winter: 0.9 } as Record<string, number>)[
            seasonal
          ] || 1.0,
        nftRarity: {
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to compute token/NFT kinetics' },
      { status: 500 }
    )
  }
}
