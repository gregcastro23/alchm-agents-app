...
import { buildMonicaPrompt, getMonicaContextPrompt, MONICA_BASE_SYSTEM_PROMPT, MONICA_SPECIALIZED_PROMPTS } from '@/lib/monica/monica-system-prompts'
...
export async function POST(req: Request) {
  ...
  if (userMsg.toLowerCase().includes('design') || userMsg.toLowerCase().includes('personalized ai')) {
  ...
  const routing = decideModel({ defaultModel: (model || process.env.MONICA_DEFAULT_MODEL) || 'gpt-4o-mini', complexity: analyzed.topicComplexity })
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // envelope
        const envelope = MonicaResponseHandler.formatResponse(text, { userMessage: userMsg, learningStage: conversationStage === 'greeting' ? 'beginner' : 'intermediate' })
        send({ type: 'envelope', payload: {
