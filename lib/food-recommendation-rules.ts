...
import type { AlchemicalProperties, ElementalProperties, ThermodynamicMetrics, AdvancedConstants } from './core-energy-rules';
...
export const FOOD_PROFILES: Record<string, FoodProfile> = {
  ...
  'Heavy Proteins': {
    ...
    contraindications: ['Negative alchemical states', 'Excess heat conditions', 'Digestive stagnation']
  },
...
export class KalchmFoodAnalyzer {
  static analyzeFoodNeeds(
    ...
    const totalAlchemical = alchemical.spirit + alchemical.essence + alchemical.matter + alchemical.substance;
    ...
    if (kalchmConstant < 0) {
      reasoning += 'Negative Kalchm constant suggests system in dynamic tension, favoring lighter foods. ';
    ...
    if (monicaConstant < 0) {
      reasoning += 'Negative Monica constant suggests resistance to change - gentle approach needed. ';
    } else if (monicaConstant > 1) {
      reasoning += 'High Monica constant supports dynamic transformation - catalytic foods beneficial. ';
    }
...
export class PlanetaryMealTiming {
  static getOptimalMealTiming(currentPlanet: string): {
    ...
    const planetaryFoodMap: Record<string, {
      mealType: string;
      foods: string[];
      preparation: string;
      timing: string;
...
export class SeasonalFoodAdjustments {
  static adjustForSeason(
    ...
    switch (season) {
      case 'Spring':
        adjusted.supportiveFoods.push('Bitter greens for cleansing', 'Fresh sprouts', 'Light detox foods');
      ...
      case 'Autumn':
        adjusted.supportiveFoods.push('Grounding foods', 'Squashes and root vegetables', 'Warming spices');
