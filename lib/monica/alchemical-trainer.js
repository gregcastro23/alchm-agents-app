...
export async function todayHourlyAlchemize(location = { latitude: 37.7749, longitude: -122.4194 }, useBatchAPI = false) {
    if (useBatchAPI) {
        try {
...
function calculateStatistics(samples) {
    ...
    statistics.correlations['spirit_heat'] = pearsonCorrelation(samples.map(s => s.alchmData.spirit), samples.map(s => s.alchmData.Heat));
    statistics.correlations['essence_entropy'] = pearsonCorrelation(samples.map(s => s.alchmData.essence), samples.map(s => s.alchmData.Entropy));
    statistics.correlations['matter_substance'] = pearsonCorrelation(samples.map(s => s.alchmData.matter), samples.map(s => s.alchmData.substance));
    return statistics;
}
