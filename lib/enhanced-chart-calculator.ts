...
export async function calculateEnhancedChart(
  ...
  try {
    ...
    const combinedAlchemical: AlchemicalData = {
      spirit: alchemicalApiData?.spirit || localAlchemicalData?.['Alchemy Effects']?.['Total Spirit'] || 0,
      essence: alchemicalApiData?.essence || localAlchemicalData?.['Alchemy Effects']?.['Total Essence'] || 0,
      matter: alchemicalApiData?.matter || localAlchemicalData?.['Alchemy Effects']?.['Total Matter'] || 0,
      substance: alchemicalApiData?.substance || localAlchemicalData?.['Alchemy Effects']?.['Total Substance'] || 0,
    ...
    throw new Error(`Chart calculation failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
