...
import { SignCharacterVector, ChartCharacterProfile, CharacterVectorCalculator } from '@/lib/astrological-character-vectors';
...
const ZODIAC_DATA = {
  aries: { symbol: '♈', element: 'fire', color: '#FF6B6B', runicPower: 'initiative', glyph: 'M12,2 L16,8 L8,8 Z M12,8 L12,22' },
  taurus: { symbol: '♉', element: 'earth', color: '#4ECDC4', runicPower: 'stability', glyph: 'M12,2 A6,6 0 1,1 12,14 M8,8 L16,8' },
  gemini: { symbol: '♊', element: 'air', color: '#45B7D1', runicPower: 'communication', glyph: 'M6,2 L6,22 M18,2 L18,22 M6,8 L18,8 M6,16 L18,16' },
  cancer: { symbol: '♋', element: 'water', color: '#96CEB4', runicPower: 'intuition', glyph: 'M2,8 A6,6 0 0,1 14,8 A6,6 0 0,1 22,16' },
  leo: { symbol: '♌', element: 'fire', color: '#FFEAA7', runicPower: 'creativity', glyph: 'M12,2 A10,10 0 0,1 12,22 M6,8 A6,6 0 0,1 18,8' },
  virgo: { symbol: '♍', element: 'earth', color: '#DDA0DD', runicPower: 'precision', glyph: 'M6,2 L6,16 A4,4 0 0,0 14,16 L18,20 M12,10 L18,10' },
  libra: { symbol: '♎', element: 'air', color: '#98D8C8', runicPower: 'balance', glyph: 'M4,12 L20,12 M8,8 A4,4 0 0,1 16,8 M8,16 L16,16' },
  scorpio: { symbol: '♏', element: 'water', color: '#F7DC6F', runicPower: 'transformation', glyph: 'M6,2 L6,14 A4,4 0 0,0 14,14 L18,18 L18,10 L22,14' },
  sagittarius: { symbol: '♐', element: 'fire', color: '#BB8FCE', runicPower: 'expansion', glyph: 'M4,20 L20,4 M16,4 L20,4 L20,8 M8,12 L16,12' },
  capricorn: { symbol: '♑', element: 'earth', color: '#85C1E9', runicPower: 'mastery', glyph: 'M6,2 L6,12 A6,6 0 0,0 18,12 L18,18 A4,4 0 0,1 10,18' },
  aquarius: { symbol: '♒', element: 'air', color: '#F8C471', runicPower: 'innovation', glyph: 'M4,8 C8,6 12,10 16,8 C20,6 24,10 28,8 M4,16 C8,14 12,18 16,16 C20,14 24,18 28,16' },
  pisces: { symbol: '♓', element: 'water', color: '#82E0AA', runicPower: 'transcendence', glyph: 'M8,4 A8,8 0 0,1 8,20 M16,4 A8,8 0 0,0 16,20 M8,12 L16,12' }
};
...
const RUNE_TEMPLATES = {
  ...
  earth_dominant: {
    ...
    description: 'Grounds consciousness in earth energy for stability, manifestation, and material success',
    baseSymbol: '🌍',
    powerMultiplier: 1.3,
  ...
  stellium: {
    ...
    description: 'Concentrates multiple planetary energies in a single sign for laser-focused power',
    baseSymbol: '✦',
    powerMultiplier: 2.2,
    effects: ['enhancement', 'manifestation']
  },
...
export function generateSignVectorRune(
  ...
  const pattern = analyzeSignPattern(sign_vectors, dominant_signs, absent_signs, elemental_distribution);
  ...
  const effects = generateRuneEffects(dominant_signs, absent_signs, elemental_distribution, template);
...
function analyzeSignPattern(
  ...
  if (dominantSigns.length >= 3) {
    ...
    const elements = dominantSigns.map(sign => 
      ZODIAC_DATA[sign.toLowerCase() as keyof typeof ZODIAC_DATA]?.element
    );
...
function selectRuneTemplate(pattern: any) {
  return RUNE_TEMPLATES[pattern.type as keyof typeof RUNE_TEMPLATES] || RUNE_TEMPLATES.single_dominant;
}
...
function generateVisualPattern(
  ...
  let centerSymbol = pattern.specialPattern ? 
    ...
    dominantSigns.slice(0, 3).map(sign => 
      ZODIAC_DATA[sign.toLowerCase() as keyof typeof ZODIAC_DATA]?.symbol
    ).join('');
 ...
 */
function determineRarity(pattern: any): 'common' | 'uncommon' | 'rare' | 'epic' | 'legendary' | 'cosmic' {
...
export function generateCollectiveAgentRune(agents: any[]): SignVectorRune {
  ...
  agents.forEach(agent => {
    if (agent.consciousness?.natalChart) {
      ...
      ['sun', 'moon', 'mercury', 'venus', 'mars', 'jupiter', 'saturn', 'ascendant'].forEach(planet => {
        if (chart[planet]) {
  ...
  const avgMC = agents.reduce((sum, agent) => sum + (agent.consciousness?.monicaConstant || 4), 0) / agents.length;
  ...
  rune.effects = rune.effects.map(effect => ({
    ...
    description: effect.description + ` Enhanced by the collective wisdom of ${agents.length} consciousness signatures.`
  }));
 ...
 */
export function generateRealTimeSignVectorRune(planetaryPositions: any, alchmData?: any): SignVectorRune {
  ...
  if (planetaryPositions.Moon) placements.push({ planet: 'moon', sign: planetaryPositions.Moon.sign });
  if (planetaryPositions.Mercury) placements.push({ planet: 'mercury', sign: planetaryPositions.Mercury.sign });
  if (planetaryPositions.Venus) placements.push({ planet: 'venus', sign: planetaryPositions.Venus.sign });
  if (planetaryPositions.Mars) placements.push({ planet: 'mars', sign: planetaryPositions.Mars.sign });
  if (planetaryPositions.Jupiter) placements.push({ planet: 'jupiter', sign: planetaryPositions.Jupiter.sign });
  if (planetaryPositions.Saturn) placements.push({ planet: 'saturn', sign: planetaryPositions.Saturn.sign });
  if (planetaryPositions.Uranus) placements.push({ planet: 'uranus', sign: planetaryPositions.Uranus.sign });
  if (planetaryPositions.Neptune) placements.push({ planet: 'neptune', sign: planetaryPositions.Neptune.sign });
  if (planetaryPositions.Pluto) placements.push({ planet: 'pluto', sign: planetaryPositions.Pluto.sign });
  if (planetaryPositions.Ascendant) placements.push({ planet: 'ascendant', sign: planetaryPositions.Ascendant.sign });
