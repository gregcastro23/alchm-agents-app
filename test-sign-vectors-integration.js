  ...
  try {
    ...
    results.forEach(result => {
      if (result.signVector && result.dominantSigns) {
        ...
        const dominantElement = Object.entries(elementalDist)
          .reduce((a, b) => elementalDist[a[0]] > elementalDist[b[0]] ? a : b)[0];
...
async function testIndividualAgentRune() {
  ...
  try {
    ...
    const response = await makeRequest(`${API_BASE}${API_ENDPOINT}?action=agent-rune&agentId=monica-001`);
    ...
    console.log(`💰 Cost: ${rune.cost.Spirit}S ${rune.cost.Essence}E ${rune.cost.Matter}M ${rune.cost.Substance}Sub`)
...
async function testSignVectorCalculation() {
  ...
  try {
    ...
    console.log(`📊 Top 3 signs: ${topSigns.map(([sign, value]) => `${sign} ${value.toFixed(1)}%`).join(', ')}`);
...
async function runAllTests() {
  ...
  const passedTests = [allAgentsResult, individualRuneResult, calculationResult].filter(Boolean).length;
