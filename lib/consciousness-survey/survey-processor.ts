...
function mapPreferredTone(choice: string): 'casual' | 'professional' | 'friendly' | 'authoritative' | 'empathetic' {
...
function mapProblemSolving(choice: string): 'systematic' | 'creative' | 'collaborative' | 'intuitive' {
...
function mapStressResponse(choice: string): 'withdraw' | 'seek_support' | 'take_action' | 'analyze' {
...
function mapConflictStyle(choice: string): 'avoid' | 'accommodate' | 'compete' | 'compromise' | 'collaborate' {
...
function getScaleValue(responses: Map<string, SurveyResponse>, id: string, defaultValue: number, reverse: boolean = false): number {
...
function hasChoiceValue(responses: Map<string, SurveyResponse>, id: string, searchText: string): boolean {
  const response = responses.get(id);
...
function calculateIndependenceCommunity(responses: Map<string, SurveyResponse>): number {
  const communityFactors = [
    ...
    hasMultiValue(responses, 'values_motivators') && 
      ...
      (responses.get('values_motivators')?.value as string[]).includes('Connection and relationships') ? 80 : 40
  ];
...
function calculateMaterialSpiritual(responses: Map<string, SurveyResponse>): number {
  const spiritualFactors = [
    hasMultiValue(responses, 'values_motivators') && 
      Array.isArray(responses.get('values_motivators')?.value) &&
      (responses.get('values_motivators')?.value as string[]).includes('Spirituality and consciousness') ? 80 : 20,
    hasMultiValue(responses, 'ai_conversation_topics') && 
      Array.isArray(responses.get('ai_conversation_topics')?.value) &&
      (responses.get('ai_conversation_topics')?.value as string[]).includes('Spirituality and consciousness') ? 70 : 30
  ];
...
function calculateArtisticLogical(responses: Map<string, SurveyResponse>): number {
  const artisticFactors = [
    ...
    hasMultiValue(responses, 'values_motivators') && 
      Array.isArray(responses.get('values_motivators')?.value) &&
      (responses.get('values_motivators')?.value as string[]).includes('Creative expression') ? 90 : 30
  ];
...
function inferInspirationSources(responses: Map<string, SurveyResponse>): string[] {
  ...
  if (hasMultiValue(responses, 'ai_conversation_topics') && 
      Array.isArray(responses.get('ai_conversation_topics')?.value) &&
...
function generateAIAdjustments(profile: ConsciousnessProfile): PersonalityInsights['ai_personality_adjustments'] {
  const adjustments = {
...
function generateConversationStarters(profile: ConsciousnessProfile): string[] {
  ...
  if (profile.meta_cognition.self_awareness > 60) {
    starters.push("I'd love to understand what makes you feel most like yourself. What activities or moments bring out your authentic self?");
  }
  
  if (profile.values.primary_motivators.includes('Personal growth and self-improvement')) {
    starters.push("What's one area of your life where you're excited about growing or improving right now?");
  }
  
  if (profile.creativity.creative_outlets.length > 0) {
    starters.push(`I notice you're drawn to ${profile.creativity.creative_outlets[0].toLowerCase()}. How does that form of expression resonate with who you are?`);
  }
  
  if (profile.thinking.problem_solving === 'intuitive') {
    starters.push("I'm curious about your intuitive side. Can you tell me about a time when your gut feeling led you in the right direction?");
  }
  
  starters.push("What's something you've been thinking about lately that you'd enjoy exploring together?");
...
function processAstrologicalFoundation(responses: Map<string, SurveyResponse>): {
  ...
  const substanceRecognition = responses.get('alchemical_substance_recognition')?.value as number || 4;
  
  const strongestPlanet = responses.get('planetary_dignity_strength_recognition')?.value as string || '';
  const challengingPlanet = responses.get('planetary_dignity_challenge_recognition')?.value as string || '';
  ...
  const estimatedAlchemicalQuantities: AlchemicalQuantities = {
    ...
    a_number: (spiritRecognition + essenceRecognition + matterRecognition + substanceRecognition) / 7,
    thermodynamic_state: thermodynamicState.includes('heating') ? 'heating' :
                        thermodynamicState.includes('cooling') ? 'cooling' :
                        thermodynamicState.includes('expanding') ? 'expanding' :
                        thermodynamicState.includes('contracting') ? 'contracting' :
                        thermodynamicState.includes('transforming') ? 'transforming' : 'stable',
    consciousness_temperature: Math.round((spiritRecognition * 15) + (essenceRecognition * 10) + (substanceRecognition * 10)),
...
function analyzeTarotArchetypalProfile(responses: Map<string, SurveyResponse>): {
  ...
  const alchemicalChoice = responses.get('alchemical_transformation_choice')?.value as string || '';
  const shadowApproach = responses.get('shadow_integration_approach')?.value as string || '';
  const energyTransmission = responses.get('energy_transmission_preference')?.value as string || '';
  ...
  const consciousnessSignature = sevenWordTruth.trim() || 'Seeking truth through authentic expression';
  ...
  if (primaryArchetype.includes('High Priestess')) traits.push('Intuitive', 'Mystical', 'Reflective');
  if (primaryArchetype.includes('Empress')) traits.push('Creative', 'Nurturing', 'Abundant');
  if (primaryArchetype.includes('Emperor')) traits.push('Structured', 'Authoritative', 'Disciplined');
...
function analyzeWritingStyle(responses: Map<string, SurveyResponse>): {
  ...
  const hasEmotionalWords = /love|joy|beautiful|wonderful|amazing|peaceful|exciting|passionate/i.test(writingSample);
...
function generatePersonalitySummary(profile: ConsciousnessProfile, insights: PersonalityInsights): string {
