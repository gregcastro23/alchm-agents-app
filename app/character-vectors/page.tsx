...
export default function CharacterVectorsPage() {
  const [userPreferences, setUserPreferences] = useState<UserLearningPreferences>(EXAMPLE_PREFERENCES);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 text-center">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">
            Character Vector System Demo
          </h1>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            This demonstrates how percentage composition of zodiac signs creates unique character vectors, 
            enabling personalized learning experiences and sophisticated chart interactions.
          </p>
        </div>
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 bg-white rounded-lg shadow-sm">
            ...
            <p className="text-xs text-muted-foreground">
              Strong Leo (Sun+Mars ~33%), Cancer Moon (20%), Sagittarius rising (23%) with Virgo Mercury. 
              Missing: Aries, Taurus, Libra, Scorpio, Aquarius, Pisces
            </p>
          </div>
          <div className="p-4 bg-white rounded-lg shadow-sm">
            <h3 className="font-semibold text-sm mb-2">Current Moment Energy</h3>
            <p className="text-xs text-muted-foreground">
              Heavy Aquarius/Pisces/Capricorn energy - offers learning in Maya&apos;s absent sign areas, 
              especially humanitarian ideals and emotional flow
            </p>
          </div>
          <div className="p-4 bg-white rounded-lg shadow-sm">
            <h3 className="font-semibold text-sm mb-2">Adaptive Learning</h3>
            <p className="text-xs text-muted-foreground">
              Fire/Earth emphasis suggests kinesthetic + visual learning, with preference for 
              direct communication and creative challenges
            </p>
          </div>
        </div>
        ...
        {selectedTrainingMode && (
          <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
            ...
            <p className="text-green-700 text-sm">
              <strong>{selectedTrainingMode.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</strong> 
              {' '}mode activated! The system would now adapt the interface, content delivery, 
              and interaction style based on Maya&apos;s character vectors and preferences.
            </p>
          </div>
        )}
