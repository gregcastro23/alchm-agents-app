...
async function testConsciousnessSurvey() {
  ...
  const sampleSurveyResponses = [
    ...
    // Creative Expression
    { questionId: 'creative_outlets', value: ['Writing and storytelling', 'Problem-solving and innovation', 'Photography and visual capture'] },
    ...
    { questionId: 'ai_conversation_topics', value: ['Philosophy and meaning', 'Psychology and human behavior', 'Personal development', 'Arts and creativity', 'Future possibilities'] }
  ];
  ...
  try {
    ...
    const getResponse = await fetch(`${baseUrl}/api/consciousness-survey?userId=${testRequest.userId}`);
    ...
    console.log('Associated AI Personalities:', getResult.consciousnessProfile.aiPersonalities.length);
...
function analyzeSurveyQuestions() {
  const { CONSCIOUSNESS_SURVEY_QUESTIONS, SURVEY_METADATA } = require('./lib/consciousness-survey/survey-questions');
