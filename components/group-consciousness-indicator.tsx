...
export function GroupConsciousnessIndicator({ 
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < agentIds.length; i++) {
      for (let j = i + 1; j < agentIds.length; j++) {
        const compatibility = GroupConsciousnessDynamics.calculateCompatibility(agentIds[i], agentIds[j])
    ...
    return (
      <Card className={className}>
        <CardHeader>
          ...
          <CardDescription>
            Select 2+ agents to see group dynamics
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  const getConsciousnessColor = (level: string) => {
    switch (level) {
      case 'transcendent': return 'text-purple-600 bg-purple-100 dark:bg-purple-950'
      case 'high': return 'text-blue-600 bg-blue-100 dark:bg-blue-950'
      case 'moderate': return 'text-green-600 bg-green-100 dark:bg-green-950'
      case 'low': return 'text-yellow-600 bg-yellow-100 dark:bg-yellow-950'
  ...
  const getMomentumIcon = (flow: string) => {
    switch (flow) {
      ...
      case 'sustained': return <Activity className="h-4 w-4 text-blue-500" />
      case 'building': return <ArrowRight className="h-4 w-4 text-yellow-500" />
      case 'dispersing': return <Target className="h-4 w-4 text-red-500" />
        ...
        {consciousnessScore && (
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              {consciousnessScore.description}
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <div className="text-lg font-bold">
                ×{groupHarmony.powerAmplification.toFixed(2)}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-sm text-blue-700 dark:text-blue-300 mt-1">
                  {groupHarmony.optimalSpeaker.replace(/-/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())} 
                ...
                <div key={index} className="flex items-center justify-between text-xs">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge 
                      variant="outline" 
                      className={`text-xs ${
                        comp.compatibility > 0.8 ? 'border-green-500 text-green-700' :
                        comp.compatibility > 0.6 ? 'border-blue-500 text-blue-700' :
                        comp.compatibility > 0.4 ? 'border-yellow-500 text-yellow-700' :
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              {groupHarmony.synergyWindows.slice(0, 3).map((window: any, index: number) => (
                <div key={index} className="flex items-center gap-2 text-xs p-2 bg-purple-50 dark:bg-purple-950/20 rounded">
