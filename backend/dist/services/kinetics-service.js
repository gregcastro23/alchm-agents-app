...
import { getPlanetaryHour } from './planetary-service.js';
/**
 * Calculate enhanced kinetics with real planetary data
 */
export async function calculateEnhancedKinetics(location, options = {}) {
    try {
        const now = new Date();
        ...
        const result = { base: baseKinetics };
 ...
 */
export function calculateGroupDynamics(agentIds, location) {
    try {
 ...
 */
export function calculateTokenKinetics(baseTokenRate, baseNFTRarity, location) {
    try {
        const now = new Date();
        ...
        if (kineticMultiplier > 1.2)
            accumulationForecast = 'Accelerating';
        else if (kineticMultiplier < 0.8)
            accumulationForecast = 'Declining';
        return {
...
// Helper functions
function getPlanetaryPower(planet) {
    const powers = {
        'Sun': 1.0,
        'Moon': 0.7,
        'Mars': 0.8,
...
function getSeasonalInfluence(dayOfYear) {
...
function calculateElementalDistribution(planetaryHours) {
    const elementalMap = {
        'Sun': 'Fire',
        'Moon': 'Water',
        'Mars': 'Fire',
...
function calculatePowerPrediction(powerData, currentTime) {
    ...
    let trend;
    if (nextHourPower > currentPower + 0.05)
        trend = 'ascending';
    else if (nextHourPower < currentPower - 0.05)
        trend = 'descending';
    else
        trend = 'stable';
...
function calculateResonanceMap(agentIds, elementalTotals) {
    const resonanceMap = {};
...
//# sourceMappingURL=kinetics-service.js.map