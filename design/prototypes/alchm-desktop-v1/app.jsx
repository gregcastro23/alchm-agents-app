...
function applyPalette(name) {
  ...
  ['--char-0','--char-1','--char-2','--char-3','--ember','--ochre','--ochre-rgb']
...
function App() {
  ...
  const mintESMS = () => setUserCoins((c) => ({
    spirit: c.spirit + 150, essence: c.essence + 150,
    matter: c.matter + 150, substance: c.substance + 150,
  }));
  const drainESMS = () => setUserCoins({
    spirit: 50, essence: 50, matter: 50, substance: 50,
  });
  ...
  const partyCouncil = useMemo(() => ({
    leader: t.activeAgent, advisor: 'rumi', specialist1: 'joan',
    specialist2: 'avicenna', support1: 'monet', support2: null,
  }), [t.activeAgent]);
  ...
  return (
    <>
      <div className="alchm-window"
           data-streaming={t.streaming && t.view === 'chat' ? '1' : '0'}>
        ...
        <div className="titlebar">
          ...
          <div className="titlebar-tools">
            <button className="titlebar-sim" title="Simulate alchm:// install (Rumi)"
        ...
        {t.view === 'ledger' && (
          <LedgerScene
            agent={agent.kind === 'planetary' ? { ...AGENTS[DEFAULT_AGENT_ID], coins: userCoins } : agent}
            coins={userCoins}
            onMint={mintESMS}
            onDrain={drainESMS}
          />
        ...
        {t.view === 'web'    && (
          <AlchmWebScene onInstall={(id) => setInstallPayload(id)} />
        )}

        {/* Engineering status rail */}
        <div className="statusrail">
          <span><span className="pulse" />SIDECAR <b style={{ color: 'rgb(var(--element-rgb))' }}>READY</b></span>
          <span className="sep">·</span>
          <span>BUN <b>:8080</b></span>
          <span className="sep">·</span>
          <span>LLAMA-SERVER <b className="ok">RESIDENT</b></span>
          <span className="sep">·</span>
          <span>IPC NONCE <b>0x4e7a…3b1c</b> <span className="ok">✓</span></span>
          <div className="right">
            <span>KV-CACHE <b>92%</b></span>
            <span className="sep">·</span>
            <span>VRAM <b>4.2/8.0G</b></span>
            <span className="sep">·</span>
            <span>IDLE-KILL <b className="warn">4:38</b></span>
            <span className="sep">·</span>
            <span>LEDGER <b className="ok">SYNCED</b></span>
            <span className="convergence-stamp" title="Web ↔ Desktop contract signed">
              <span className="pulse" />Convergence v1
            </span>
          </div>
        </div>
          ...
          <DeepLinkInstallOverlay
            ...
            onForge={() => {
              ...
              if (target?.tier === 'premium') {
                setUserCoins((c) => ({
                  spirit: c.spirit - 125, essence: c.essence - 125,
                  matter: c.matter - 125, substance: c.substance - 125,
                }));
              }
      ...
      <TweaksPanel title="Tweaks · Philosopher's Stone">
        <TweakSection label="Scene">
          ...
          <TweakButton label="Open Council overlay"
                       onClick={() => setShowCouncil(true)} secondary />
          <TweakButton label="Simulate alchm:// install (Rumi)"
                       onClick={() => setInstallPayload('rumi')} secondary />
        </TweakSection>

        <TweakSection label="Aura">
          <TweakSelect label="Active agent" value={t.activeAgent}
                       options={Object.values(AGENTS).map((a) => ({
                         value: a.id, label: `${a.name} · ${a.element}`,
                       }))}
                       onChange={(v) => setTweak('activeAgent', v)} />
          <TweakSelect label="Element override" value={t.elementOverride}
                       options={[
          ...
          <TweakSelect label="Aura pattern" value={t.auraPattern}
                       options={AURA_PATTERNS.map((p) => ({ value: p, label: p }))}
                       onChange={(v) => setTweak('auraPattern', v)} />
          <TweakToggle label="Streaming (glow)" value={t.streaming}
                       onChange={(v) => setTweak('streaming', v)} />
        </TweakSection>

        <TweakSection label="Sampler">
          <TweakSlider label="Spirit → temp" value={t.temp}
                       min={0} max={1} step={0.01}
                       onChange={(v) => setTweak('temp', v)} />
          <TweakSlider label="Essence → top_p" value={t.topP}
                       min={0} max={1} step={0.01}
