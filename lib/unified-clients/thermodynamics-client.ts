...
function calculateThermodynamicsFallback(values: ElementalValues) {
  ...
  const heat =
    denominator > 0 ? (Math.pow(spirit, 2) + Math.pow(fire, 2)) / Math.pow(denominator, 2) : 0
...
class BackendThermodynamicsClient {
  ...
  static async calculateThermodynamics(
    elementalValues: ElementalValues
  ): Promise<BackendThermodynamicsResponse> {
...
export class UnifiedThermodynamicsClient {
  private static useBackend =
    typeof process !== 'undefined' && process.env.NEXT_PUBLIC_THERMODYNAMICS_BACKEND === 'true'
