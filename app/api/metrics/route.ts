...
async function generateMetrics() {
  ...
  return {
    ...
    evolution: {
      ...
      levels: evolutionLevels.reduce((acc, level) => {
        acc[level.currentLevel] = level._count
        return acc
      }, {} as Record<string, number>)
    },
    subscriptions: {
      tiers: subscriptionTiers.reduce((acc, tier) => {
...
function formatPrometheusMetrics(metrics: any): string {
  ...
  metrics.agents.popular.forEach((agent: any) => {
    lines.push(`planetary_agents_agent_interactions{agent_id="${agent.agentId}"} ${agent.interactions}`)
  })
