  ...
  execute: async ({ planet, sign, source }) => {
    ...
    const interpretations: Record<string, Record<string, Record<string, string>>> = {
      Ptolemy: {
        ...
        Moon: {
          Cancer: "The Moon in Cancer according to Ptolemy indicates great sensitivity and intuition.",
          // Add more signs
        },
        // Add more planets
      },
      Lilly: {
        Mars: {
          Aries: "Mars in Aries according to William Lilly indicates a valiant and courageous nature.",
          Scorpio: "Mars in Scorpio according to William Lilly indicates a secretive and strategic warrior.",
          // Add more signs
        },
        // Add more planets
      },
  ...
  execute: async ({ birthInfo }) => {
    ...
    return {
      ...
      alchemicalInfo: {
        ...
        sunSign: alchemicalInfo['Sun Sign'] || mockHoroscope.tropical.CelestialBodies.all[0].Sign.label,
        chartRuler: alchemicalInfo['Chart Ruler'] || getRulingPlanet(mockHoroscope.tropical.CelestialBodies.all[0].Sign.label),
