  ...
  for (const agent of HISTORICAL_AGENTS) {
    try {
      ...
      const agentData = {
        ...
        wisdomDomains: (agent as any).wisdomDomains ||
          agent.abilities?.wisdomDomains || [agent.specialization],
        skills: agent.personality.traits || [],
        teachingStyle: (agent as any).teachingStyle || agent.abilities?.teachingStyle || 'Adaptive',
        resonanceType:
          (agent as any).resonanceType || agent.abilities?.resonanceType || 'Universal',
        uniquePower:
          (agent as any).uniquePower ||
...
seedHistoricalAgents().catch(error => {
  console.error('\n❌ Fatal error:', error)
  process.exit(1)
})
