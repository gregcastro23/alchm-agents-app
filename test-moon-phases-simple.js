...
async function testMoonPhaseAPI() {
  ...
  try {
    ...
    if (data.success) {
      ...
      console.log(`   Sign: ${data.agent.phase.zodiacSign} at ${Math.round(data.agent.phase.zodiacDegree)}°`);
  ...
  for (const combo of testCombos) {
    try {
      ...
      if (data.success) {
        console.log(`   ${data.agent.phase.emoji} ${combo.phase} in ${combo.sign} ${combo.degree}°:`);
